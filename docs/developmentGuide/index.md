...
KWC（Kingdee Web Components）是一个具有自己 API 的可重用的自定义 HTML 元素，是金蝶云苍穹平台基于现代Web Components标准构建的企业级前端组件解决方案。它继承并扩展了原生Web Components技术，为金蝶生态系统的前端开发提供了一套标准化、可复用、高性能的组件开发范式。我们可以使用KWC框架在苍穹平台上构建自定义用户界面、Web和移动应用程序。

### KWC优势

**Web Component** 作为浏览器原生支持的组件化标准，其「框架无关、独立封装、原生兼容」的核心特性，为跨项目、跨技术栈的组件复用提供了天然优势。

KWC 基于Web Component标准，完整继承了这些特性：每个 KWC 组件都具备独立的 DOM 结构、隔离的样式空间与专属 API，可脱离 Vue、React 等具体前端框架独立运行，既能彻底解决多项目间组件重复开发的冗余问题，又能有效规避不同技术栈带来的组件兼容、样式冲突等痛点，真正实现「一次开发、多端复用、跨栈集成」的开发目标。

与此同时，KWC 并非对 Web Component 原生能力的简单复用，而是结合金蝶云苍穹平台的企业级业务场景与开发需求，进行了全方位的定制化增强：

*   通过简化的语法设计、统一的开发规范，屏蔽了 Web Component 原生 API 的繁琐细节，大幅降低开发者的学习门槛与开发成本，让开发者更专注于业务逻辑实现；
*   深度适配金蝶云苍穹平台的技术体系，与平台现有工具链、系统架构无缝衔接，确保组件在复杂企业级项目中能够高效落地、稳定运行，满足平台对组件性能、安全性、可维护性的严苛要求。

KWC的优势，还可简单总结如下：

**学习成本低**

开发者编写的大部分代码都是标准的 JavaScript 和 HTML，通过API与后端通信，快速上手，无需额外学习其他框架。
...
一套组件跨PC、移动等多端运行，统一交互标准，利用浏览器原生渲染能力，显著提升性能与用户体验。
...
1.  首先需要安装node.js，并确保 node 版本是 18 或以上。

到 [node 官网](https://nodejs.cn/download/)下载，或者使用 nvm 管理 node 版本，安装完成后能正确查看到版本即安装成功。
```bash
$ node -v
v20.17.0
$ npm -v
...
1.0.4
```
1.  还需要安装git工具，可到[git官网](https://git-scm.com/install/windows)下载安装，在创建项目模板时，从代码仓库中下载模板文件。
...
使用 kd-custom-control-cli create \[自定义控件方案id] 创建一个新的自定义控件工程，注意这里的自定义控件名称和设计器中**方案id**保持一致，输入格式只能包含小写字母和数字。执行创建命令后，我们选择KWC框架。
```bash
...
```
在浏览器里打开http\://localhost:8000/，能看到以下界面，代表项目启动成功：

![](https://wcnxho25elt6.feishu.cn/space/api/box/stream/download/asynccode/?code=Zjk3ZDgzMmU2ZDA0Nzk2NmM4YjZmNmVhZWQ4MGEyNDZfdm9BR0NwM1JPcmwwVjhuUU5sYnIzeE1EenRxbG5qMUZfVG9rZW46S0E0MmJRSG9nb24yQmx4V2VBMGNwbDdsbjBiXzE3NjcxNjY2MjU6MTc2NzE3MDIyNV9WNA)
...
将组件文件夹命名为 `myComponent`。组件文件夹的驼峰命名会在HTML标签中被处理为连字符形式。例如，在 html 文件中，我们要使用 `modlues/x/myComponent`这个组件，我们需要使用 `x-my-component`:
```html
<!-- parent.html -->
<template>
...
![](https://wcnxho25elt6.feishu.cn/space/api/box/stream/download/asynccode/?code=Yjk5MGRlYzA2OGJhZWU0MTE3ZDc0ZDFhMGNjYTkwYjJfUXAzcEFpN0M3eUlLUk1pVlI5ejltRkNtUUpZQjJGRHhfVG9rZW46TEdremJ0QWw5b3p6Skt4SjMyMWNFZjY4bllkXzE3NjcxNjY2MjU6MTc2NzE3MDIyNV9WNA)

![](https://wcnxho25elt6.feishu.cn/space/api/box/stream/download/asynccode/?code=NjMxMThlNWVkNTQ5YmYxNGQ5NjVhNDliMzhkMGViMTJfMUp1bTByNTVsbGxYSU9qWndOVjg4QTN0TXQxZ0ZXVmRfVG9rZW46UVYzS2JWNDY4b3RIcEF4UzBSbmNpUERnblNlXzE3NjcxNjY2MjU6MTc2NzE3MDIyNV9WNA)
...
个组件都必须要有一个 JavaScript 文件。如果组件有其 UI 元素，那么 JavaScript 文件则定义了 HTML 元素相关的属性以及交互逻辑。如果组件只是 API 模块，那么 JavaScript 文件则负责导出功能，以便其他组件使用。
...
目前在自定义控件中支持KWC框架，因此使用`@kdcloudjs/kd-custom-control-cli`命令创建KWC项目。

当使用kd-custom-control-cli命令创建KWC项目后，会自动生成与项目名对应的元数据文件：myComponent.js-meta.kwc。
...
![](https://wcnxho25elt6.feishu.cn/space/api/box/stream/download/asynccode/?code=NDc5MGRlODAzMDE4NmFjZGMxMDkwYWRkM2I1MDM0MjFfbXhBM0l6S1QyRDJrZ3l2SlRhRlBISkZFMk5GWktjMUVfVG9rZW46WDFkM2JrbDN6b2VCaFR4cnpqdWN6VEVBbmpjXzE3NjcxNjY2MjU6MTc2NzE3MDIyNV9WNA)

![](https://wcnxho25elt6.feishu.cn/space/api/box/stream/download/asynccode/?code=MTM3YmMzMzk3ZTI2MzkzYjU1MDg2OTM0YjczNzVkZDRfeHZySTBxeno3Q3F4bVlyU3BaMWNuZExNNGlYYUhzWEVfVG9rZW46VEVtcGI4czRvb2NycUd4emk2NGMwRXFMbjlkXzE3NjcxNjY2MjU6MTc2NzE3MDIyNV9WNA)
...
![](https://wcnxho25elt6.feishu.cn/space/api/box/stream/download/asynccode/?code=YzdkY2I4NjJjYTFmMWIyODNkYzhiZmUzMDVkZDNjNzhfNnBtWW9hTktSR3lLYllKR1ZrVkZSN0xKZ1FoUmJURWdfVG9rZW46R3hjUmJHVDgyb0h0S1d4UU5KT2NERnNabmVmXzE3NjcxNjY2MjU6MTc2NzE3MDIyNV9WNA)

![](https://wcnxho25elt6.feishu.cn/space/api/box/stream/download/asynccode/?code=NWVlNDAwNmQ4N2U1NmY4MDRiMDgyZGIzNzdhMTdiOGZfRG1CNVV1UEJZaDl3QllsME15eUptSmhJQ25WTzBWdjZfVG9rZW46TXVhZWJOekE0b2tZcUh4Y0Rrb2NkaTB5bk1lXzE3NjcxNjY2MjU6MTc2NzE3MDIyNV9WNA)
...
```
![](https://wcnxho25elt6.feishu.cn/space/api/box/stream/download/asynccode/?code=OTNiZTFkYzY5NzAxNGZkMzkzNjI3ZTg2YjdmYjg0YzhfemhkclJpWmdnNmczUHZWVG40Rk1mSllWY25UZVYzWnBfVG9rZW46RnlCQWJmZmFvb0NjZDR4TlNCZmNTc0J0bmNjXzE3NjcxNjY2MjU6MTc2NzE3MDIyNV9WNA)
...
```
![](https://wcnxho25elt6.feishu.cn/space/api/box/stream/download/asynccode/?code=ZjMyZDI5Njg1YjgwNjE3NjY3MGUxYzA5NWE0ZmNjOWRfQm94a3p5Q21ueG1VWGRVSmlLVUFTSnk4U1N5VjkzRTZfVG9rZW46RUpxS2Jvdm1wb3VmVE14SjY5TGNtVm5kbkNjXzE3NjcxNjY2MjU6MTc2NzE3MDIyNV9WNA)
...
![](https://wcnxho25elt6.feishu.cn/space/api/box/stream/download/asynccode/?code=MWI2ZjdjZDRkOTVkMjk5NmE0YmQ3YTA2NGQ1MWY2MDhfQUtPUXV1OWZiTFZiWERIZ1hyWUhIQkRCeGlkRGhBVTRfVG9rZW46SmFYVWJiaXR6b1o0SXZ4dTJBWmNteUlPbnVYXzE3NjcxNjY2MjU6MTc2NzE3MDIyNV9WNA)
...
```
![](https://wcnxho25elt6.feishu.cn/space/api/box/stream/download/asynccode/?code=MzJlZDkzMDY5MGQ0M2UzZWE1MzcyZjZmZDc0YWVjMjlfY3ljNjRPa0NXVG1HS2ZJV1VaMUJ5ZGQzN3pUYXhJUzlfVG9rZW46TDRnMmJ3WGtEbzdWQTd4YURJd2NabHFabmVmXzE3NjcxNjY2MjU6MTc2NzE3MDIyNV9WNA)
...
KWC组件封装并简化了组件内部、跨级和组件与组件间的通信，详细内容可参考相关章节：

*   属性（Properties)
*   自定义事件（Events)
*   跨级通信

    *   事件冒泡（bubbles和composed）
    *   发布订阅模式（Pub/Sub）：KWC提供的messaService工具
    *   访问组件元素
    *   Shadow DOM
    *   插槽（Slot）
...
*提示：装饰器，如`@api`，`@track`是 JavaScript 的一个功能。一个属性声明只能有一个装饰器。*
...
KWC 组件中，可以访问Node 元素的以下属性：`childNodes`, `firstChild`, `isConnected`, `lastChild`
...
KWC会分发标准的 DOM 事件。组件还可以创建和分发自定义事件。使用事件可以向上层组件进行通信。例如，子组件 `c-todo-item` 会分发一个事件来通知其父组件 `c-todo-app` ，用户已选中该组件。

KWC中的事件是基于 [DOM 事件](https://dom.spec.whatwg.org/#events)构建的，DOM 事件是每个浏览器中都可用的 API 和对象的集合。
...
在 KWC中，要创建事件，我们强烈建议使用 `CustomEvent` 接口， `CustomEvent` 可在各种浏览器上提供更一致的体验。它无需任何设置或样板代码，并且允许您通过 `detail` 属性传递任何类型的数据，因此非常灵活。KWC实现了 `EventTarget` 接口，这使得它们能够分发事件、监听事件和处理事件。
...
以上绑定事件监控器，都是声明式的，KWC也支持显式（命令式）绑定事件监控器。
...
添加事件监听器有两种语法。一种是将事件监听器添加到组件shadow边界内的元素，另一种是将事件监听器添加到模板不拥有的元素，例如，传递给插槽的元素。

要向shadow边界内的元素添加事件监听器，请使用 `template` 。
```js
...
值得注意的是，对于 `c-todo-item` 上的监听器， `Event.target` 是 `c-todo-item` ，而不是 `div` ，因为 `c-todo-item` 位于shadow边界之外。
...
注意 *修改通过模板定义的元素的属性可能会对组件的其他部分产生意想不到的副作用。在我们的示例中，元素的输入值属性从模板中定义的值（由 `evt.target` 表示）发生了变化。该示例更改了模板中使用的值（\` `myValue` ），以保持组件元素的状态同步。否则，模板重新水合（template rehydration）在尝试将输入元素的状态与组件的状态进行匹配时，会检测到属性值不匹配。这种不匹配会生成运行时警告，您需要修改组件或 JavaScript 代码，以确保整个模板中的数据完整性。*
...
&#x20;一个布尔值，指示事件是否可以穿过shadow边界。默认为 `false` 。
...
#### bubbles\:true和composed\:false

事件会向上冒泡穿过 DOM，但不会越过shadow边界。因此， `c-child` 和 `div.wrapper` 都可以响应该事件。
```html
<body>
    <c-app>
    #shadow-root
...
要在组件模板内部冒泡事件，请在模板中的某个元素上分发该事件。事件只会向上冒泡到模板内该元素的祖先元素。当事件到达shadow边界时，它就会停止冒泡。
...
```
#### bubbles\:true和composed\:true

事件会向上冒泡穿过 DOM，越过shadow边界，并继续向上冒泡穿过 DOM 到达文档根目录。
...
#### bubbles\:false和composed\:true

KWC组件不使用此配置。

### 组件间消息中心（messageService）

KWC框架中，提供了messageService，messageService可用于组件之间的通信，支持本地与跨页签广播。
```js
...
在 kwc 框架中，我们使用合成shadow的方式来模拟原生 Shadow DOM 的行为，以便支持旧版浏览器

#### 合成shadow的组件渲染

在使用合成shadow渲染组件时，KWC 会添加一个属性来模拟原生组件的行为

假设你有一个 `kd-button`组件，在浏览器中渲染结果如下

![](https://wcnxho25elt6.feishu.cn/space/api/box/stream/download/asynccode/?code=OTFhMTY4MWE4MDliMDhjY2ExYzUyZGY4ODJjZWE2NzVfUmRnak8xMXF4MW9IYUE0OFAyU0Z3OUlRQVNITXh1TmxfVG9rZW46T0lxcmJSVXROb2todjJ4NlZ2MGNXTEVEbmRmXzE3NjcxNjY2MjU6MTc2NzE3MDIyNV9WNA)
...
然而，传递给插槽的 DOM 元素并不属于组件的shadow tree。要访问通过插槽传递的元素，组件会调用 `this.querySelector()` 和 `this.querySelectorAll()` 。
    ...
    <template>
      <div class="parent">
        <template for:each={itemsData} for:item="itemData">
          <x-child onclick={onItemSelect} id={itemData.id} key={itemData.id}></x-child>
        </template>
      </div>
    </template>
```
...
`connectedCallback()`生命周期在组件插入 DOM 树时触发。`disconnectedCallback()`在组件中 DOM 中移除或者隐藏时触发。这两个狗子函数都是遵循父组件到子组件的流程。当需要访问宿主元素时，使用 `this`。要访问模板的话使用 `this.template`。要检查组件是否连接到 DOM，可以使用 `this.isConnected`.
...
![](https://wcnxho25elt6.feishu.cn/space/api/box/stream/download/asynccode/?code=ZWU4MTJjNzg2NjliMGVlZjdmY2ZlZGE5NTc2NWVlOTZfZUVuTFdSVXlvQlFCeno1RndibEtMc2tTdkVpWlc3QUpfVG9rZW46WUNtUWJxTnFab0pCcXB4RVM3bmNXcmhDblhlXzE3NjcxNjY2MjY6MTc2NzE3MDIyNl9WNA)
...
export default class extends KingdeeElement {
    childProps = { name: 'Kingdee Web Components', onclick: this.handleClick.bind(this) };
...
组件元数据是对组件及组件部署的描述。下面是自定义控件KWC框架的组件元数据：
...
当在KWC组件项目根目录下，执行`kd-custom-control-cli deploy`命令上传组件元数据时，会提示输入方案名称、领域标识和开发商标识，命令执行后将会填充相应字段。
...
| MobileBillFormModel | 移动单据，会自动包含BaseFormModel与BillFormModel |
...
当在本地自定义控件，KWC项目目录中，执行`npm/yarn install` ，然后再执行`npm/yarn start` 命令启动本地服务，即可进行本地预览和调试。
...
当本地调试时，可以通过kd-custom-control-cli命令将控件xml元数据文件部署到指定苍穹环境中，用于远程联调（即：远程苍穹环境与本地前端代码进行联调），这样可省去打包上传的操作，使开发调试更便捷。

上传控件元数据操作，在myComponent项目根目录下执行kd-custom-control-cli deploy命令，然后根据提示输入信息：
...
| client\_id     | 在上述苍穹环境上创建的OpenAPI第三方应用的client\_id     |
| client\_secret | 在上述苍穹环境上创建的OpenAPI第三方应用的client\_secret |
| username       | 在上述苍穹环境上创建的OpenAPI第三方应用的username       |

注：输入OpenAPI第三方应用信息的目的是进行苍穹环境登录认证，以便将xml元数据文件部署到该环境，可参考文章：[OpenAPI第三方应用注册流程](https://developer.kingdee.com/knowledge/525972550665201152?productLineId=29\&isKnowledge=2\&lang=zh-CN)。
...
1、修改 \[项目根目录]/server/config.js 文件中的localServer 属性值为true

2、在项目根目录下执行 `npm run build` 启动本地动态构建服务。

**执行远程联调**

打开远程苍穹环境的对应表单预览页面，在该设计器页面或预览页URL地址后拼接上本地静态服务器地址`&kdcus_cdn=http://localhost:3001` 进行调试，驰可预览效果。

![](https://wcnxho25elt6.feishu.cn/space/api/box/stream/download/asynccode/?code=YmEzMTI3NWI1ODY1ZjNmYzU0MGMwZTVlODg0ZDdmY2NfQTJwZXZqenBXZW0waGdwekl2N2hGd013enVhR0RrbFFfVG9rZW46SVJFbWJhRUJrb3pXbDl4c3BCV2NqVzJWbmFlXzE3NjcxNjY2MjY6MTc2NzE3MDIyNl9WNA)

![](https://wcnxho25elt6.feishu.cn/space/api/box/stream/download/asynccode/?code=ZjEwMGQyYTJmY2NjNDhiZTdjYWY1OTVjNTkzMjUwYzFfeEtza2FkZXNKcWFtazFCWHBQMkVqZjZ6UHNNRVZUV2xfVG9rZW46Q2RQc2JsRVY3b0ZKcU94U08wMmNDQW1Xbm5nXzE3NjcxNjY2MjY6MTc2NzE3MDIyNl9WNA)

当更新了本地KWC代码时，则手动刷新该页面即可查看更新内容。

注意：若修改了XML元数据文件，则需要执行 kd-custom-control-cli deploy -f，将元数据文件重新部署到远程环境上。
...
```js
    ...
    const asyncFallback = await tAsync('non.existent.key', { fallback: 'Fallback', path: 'path/to/en_US.json' });
```
#### 6. 清除翻译缓存
```js
    // 清除指定语言的翻译缓存
...
```js
    // ES Module 导入
    import { client, apiAdapter, openApiAdapter, platformResourceLoader } from '@kdcloudjs/kwc-shared-utils'
...
// 加载外部脚本
platformResourceLoader.loadScript(document.head, 'https://example.com/script.js')
    .then(url => {
...
// 加载外部样式表
platformResourceLoader.loadStyle(document.head, 'https://example.com/style.css')
    .then(url => {
...
// 使用回调方式
platformResourceLoader.loadScript(document.head, 'https://example.com/script.js', (error, url) => {
    if (error) {
