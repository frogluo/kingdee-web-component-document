...
在浏览器里打开http\://localhost:8000/，能看到模板展示内容，代表项目启动成功。

![b4f39c2927797d7422bd632231cc3384__preview_type=16.png](https://tc-cdn.processon.com/po/5cd95fb0e4b06c0492ed0920-695a13c7c5602355134dc751)
...
响应式是 KWC 框架的核心系统，该框架会监听属性值的变化。当监听到变化时，会重新根据模板中使用的表达式来重新渲染组件，显示新值。

组件模板中的公共属性是响应式的。在组件的 JavaScript 类中，如果某个属性值发生变化，并且该字段在模板中或者是被其他 getter 依赖时，组件会重新渲染并显示新值。如果该属性是个对象或者数组，那么框架则会监听其内部的变化。

在KWC中，提供了装饰器`@api`和`@track`来监听属性变化，@api用于公共属性，@track用于私有属性。使用时，一个属性声明只能有一个装饰器。
...
*   `@track` 仅作用于组件的实例属性，无法用于静态属性（`static` 修饰的属性），静态属性的变化不会触发组件重新渲染。
*   即使使用 `@track`，也需遵循 “不可变数据” 的最佳实践（尤其是数组 / 对象），直接修改内部数据不如替换整个数据对象（如 `this.userInfo = { ...this.userInfo, age: 26 }`），能让响应式行为更稳定。
*   与 `@api` 装饰器的区别：`@api` 标记的是公共属性（可被父组件传递数据）；`@track` 仅标记私有 / 保护属性的响应式，不具备暴露给父组件的能力。
...
该框架以递归方式监听对对象和数组的修改，包括嵌套对象，嵌套数组以及对象和数组的混合。此外，它还能处理循环引用。

但是框架不会监听复杂对象的变异操作，例如 `Date`等。
...
`@track`的另一个用途是告诉框架监听数组元素的变化。如果不使用装饰器的话，框架仅会监听数组重新赋值时的变化，为数组添加新值和更新内部元素不会引起组件渲染。
...
```
<kd-button kwc-190pm5at4mo kwc-nfvqqei982-host style="
    /* display: flex; */
    /* height: 40px; */
  "
>
  <button kwc-nfvqqei982 class="kdds-button kdds-button-variant-primary kdds-button-shape-radius kdds-button-size-medium">
...
KWC 会在组件上生成混淆字符串作为 `kwc-nfvqqei982`属性，用于限定 CSS 在组件内部的作用域，从而模拟原生 Shadow DOM 的行为。如果你为 `kd-button`提供样式的话，那么 CSS 作用域标记会在运行时添加到元素上。
...
![a7cff849ecb62bdc3645ffa7eb27d6c1__preview_type=16.png](https://tc-cdn.processon.com/po/5cd95fb0e4b06c0492ed0920-695a1797af380f3b56b0270e)
